...
public class Paging {
	...
	private int offset = 0 ;	//건너뛰어야할 레코드 갯수 ex) 내가 4페이지를 보려면 몇 개를 건너뛰고 봐야하느냐? ("4개" 왜냐면, record가 한 페이지당 2개니까)
	private int limit = 0 ;
	private String url = "" ;
	private String pagingHtml = "";//하단의 숫자 페이지 링크의 정보가 전부 들어있다.
	...
	private String getPagingHtml( String url ){ //페이징 문자열을 만든다.
		...
		//&whatColumn=null &keyword="%test%"
		
		
		//앞, 뒤 는 페이지 갯수가 3개가 넘어갈 시에 생성된다.
		
		//앞쪽
		
		// 시작페이지가 1이 아니라면,(4,5,6) [맨 처음] 이라는 링크설정 주겠다.
		// 시작페이지가 1이 아니라면, [이전] 이라는 링크설정 주겠다.
		...
		return result ;
		// => getPagingHtml(url) ;에서 요청을 했으니 리턴하고 그게 this.pagingHtml에 담긴다.
	}   
